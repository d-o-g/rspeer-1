...
public class AbstractNode extends Tree<AbstractNode> implements Opcodes {
    ...
    public AbstractNode first(Predicate<AbstractNode> filter) {
        for (AbstractNode n : this) {
            if (filter.test(n))
                return n;
        }
    ...
    public boolean hasChild(Predicate<AbstractNode> filter) {
    ...
    public boolean parentIs(int opcode) {
        return hasParent() && parent().opcode() == opcode;
    }
}
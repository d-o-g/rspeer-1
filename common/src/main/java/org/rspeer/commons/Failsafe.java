...
public class Failsafe {
    ...
    private static FailsafeExecutor<Object> getExecutor(int tries, int delay, Class<? extends Throwable> exception,
        ...
        RetryPolicy<Object> retryPolicy = new RetryPolicy<>()
                ...
                .onRetry(onRetry)
                .abortOn(s -> isShuttingDown)
                .withMaxRetries(tries);
        return net.jodah.failsafe.Failsafe.with(retryPolicy);
    }

    public static<T> T execute(ContextualSupplier<T> supplier, int tries, int delay, Class<? extends Throwable> exception,
                               CheckedConsumer<? extends ExecutionAttemptedEvent<Object>> onRetry) {
    ...
    public static<T> T execute(ContextualSupplier<T> supplier, int tries,
                               CheckedConsumer<? extends ExecutionAttemptedEvent<Object>> onRetry) {
    ...
    public static<T> T execute(ContextualSupplier<T> supplier, CheckedConsumer<? extends ExecutionAttemptedEvent<Object>> onRetry) {
        return execute(supplier, DEFAULT_RETRIES, DEFAULT_RETRY_RATE, DEFAULT_EXCEPTION, onRetry);
    }

    public static<T> T execute(ContextualSupplier<T> supplier) {

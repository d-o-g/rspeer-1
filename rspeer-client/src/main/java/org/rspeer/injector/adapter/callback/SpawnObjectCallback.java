...
public final class SpawnObjectCallback extends MediatorDelegate<MethodHook> {

    private static final String SPAWN_OBJECT_LATER_DESC = "(IIIIIIIII)V";

    public SpawnObjectCallback(Modscript modscript, Map<String, ClassNode> library) {
        super(modscript, modscript.classes.get("Client").getMethod("spawnObjectLater"), library);
    }
    ...
    public void visitMethodNode(MethodNode mn) {
        InsnList instructions = new CodeGenerator().append(referenceMediator())
                ...
                .loadLocal(ILOAD, 7)
                .loadLocal(ILOAD, 8)
                .invokeVirtual(-1, MEDIATOR, "spawnObjectLater", SPAWN_OBJECT_LATER_DESC)
                .collect();
